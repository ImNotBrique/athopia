...
bot.on('ready', function() { 
 ...
 console.log('Je suis connecté :D') 
    ...
    bot.user.setActivity(`${bot.guilds.size} serveurs | Prefix : +`, { type: 'Listening'});
...
message.channel.send(e)
  ...
  //`${Date.now() - message.createdTimestamp}`
    msg.edit(new Discord.RichEmbed().addField('Voici le ping du bot !', ` :ping_pong: Pong ! ${Date.now() - message.createdTimestamp} ms `).setFooter('©️ AthopiaTEAM | Powered by GlitchBot Creator').setColor('RANDOM'))
  });
}
 ...
 if(message.content === prefix + 'serverinfo'){
  ...
  if(message.content === prefix + "partenaire"){
    var e = new Discord.RichEmbed()
   .addField('Discord des partenaires', '[Serveur Discord](https://discord.gg/PkbhD3f)')    
      .setColor('RANDOM')   
    .setFooter('©️ AthopiaTEAM | Powered by GlitchBot Creator')
    message.channel.send(e)
  }
   ...
   if(message.content === prefix + "help"){ //xd xd
    if(!message.guild) return message.channel.send('Une erreur s\'est produite !')
    var e = new Discord.RichEmbed()
    .setTitle('Help | Aide')
    .addField('__**Fun**__', `!ping :arrow_right: Ping du BOT`)
  ...
  // ICI LES COMMANDES DE +WORK ET TOUT MON P'TIT ADAM HéHéHoHoHaHaHuHuHiHi
});
